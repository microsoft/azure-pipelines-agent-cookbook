provides :azure_pipelines_agent
...
action :install do
  version = new_resource.version || node['azure_pipelines_agent']['binary']['version']

  ruby_block "Restart azure_pipelines_agent #{new_resource.agent_name} service" do
    block do
  ...
  template "#{new_resource.install_dir}/.path" do
    ...
    mode '0755' unless windows?
    action :create
    cookbook 'azure_pipelines_agent'
    notifies :run, "ruby_block[Restart azure_pipelines_agent #{new_resource.agent_name} service]", :delayed if new_resource.runasservice
...
action :restart do
  if current_resource && current_resource.install_dir && current_resource.runasservice
    converge_by("Restarting agent '#{current_resource.agent_name}'") do
      ruby_block "Restart azure_pipelines_agent #{current_resource.agent_name} service" do
        block do
...
action_class do
  ...
  def remove_agent(resource)
      ...
      ruby_block "Disable azure_pipelines_agent['#{resource.agent_name}'] service" do
          block do
